...
inportFile:
	...
	la   $a0, fileName 		# upload file name
...
fillDictionaryArray:
	...
	sw $a0, ($a1)					# Store scanner position as a pointer in the dictionaryArray   
	add $v1, $0, $0					# initialize to 0
...
# }
# This is a single function with multiple parts. 
...
# {
	...
	fillDictionaryArrayCorrectLoop:
		get_WordArray ($a0, $s1, $s4)  				# Ethan and Anshul can you double check my loop - Shah
		lb $t0 ($a0)						# Looks ok here. It runs so no real worries -Ethan
...
getNineLetterReturn:
	...
	addi $sp, $sp, 4
	jr $ra
# Executes a randomization of the Word in the box.
randomizeWord:
	addi	$sp, $sp, -4
	sw	$ra, ($sp)
	la	$s1, wordInBox
...
continueRandomization:	
	beq	$t0, $t1, randomizeWordLoop 		# if t1 is the same as t0, jump to randomizewordloop, else, switch characters
...
# Does what it says. Gets the center character all words have to include.
getKeyLetter:
	la $t1, wordInBox
	lb $t1, 4($t1)
	sb $t1, keyLetter
